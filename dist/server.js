  ...
  redisSubscriber
    ...
    .then(() => {
      logger_1.logger.info(
        `Subscribed to Redis channel ${EXECUTION_EVENTS_CHANNEL}`,
      );
    })
    .catch((error) => {
      logger_1.logger.error(
        "Failed to subscribe to Redis execution event channel",
        {
          error,
        },
      );
...
app.get("/health/advanced", async (req, res) => {
  try {
    const metrics =
      await workflowMonitoring_1.workflowMonitoring.getHealthMetrics(1); // Last hour
    res.json({
      status: "ok",
