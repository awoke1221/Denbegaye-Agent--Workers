...
const setupWebhookRoutes = (app) => {
    ...
    app.post("/api/webhooks", verifyToken, async (req, res) => {
        try {
            const userId = req.user?.id;
            const { name, url, event_type, agent_id, headers, is_active } = req.body;
    ...
    app.get("/api/webhooks/:id", verifyToken, async (req, res) => {
        try {
            ...
            logger_1.logger.error("Error fetching webhook", { error });
    ...
    app.post("/api/webhooks/:id/test", verifyToken, async (req, res) => {
        try {
            const userId = req.user?.id;
            const { id } = req.params;
            const { data: webhook, error: getError } = await supabaseClient_1.supabase
                .from("webhookTriggers")
                .select("*")
                .eq("id", id)
                .eq("user_id", userId)
    ...
    app.get("/api/webhooks/:id/events", verifyToken, async (req, res) => {
        try {
            const userId = req.user?.id;
            const { id } = req.params;
            ...
            const { data: events, error } = await supabaseClient_1.supabase
                ...
                .range(Number(offset) || 0, (Number(offset) || 0) + (Number(limit) || 50) - 1);
            if (error) {
                logger_1.logger.error("Failed to fetch webhook events", { error });
                return res.status(500).json({ error: error.message });
            }
            res.json({ events: events || [] });
        }
        catch (error) {
            logger_1.logger.error("Error fetching webhook events", { error });
