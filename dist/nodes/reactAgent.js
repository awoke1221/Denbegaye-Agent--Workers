...
exports.reactAgentHandler = void 0;
...
const hasExpectedReActKey = (value) => isObject(value) &&
    EXPECTED_REACT_KEYS.some((key) => Object.prototype.hasOwnProperty.call(value, key));
const cleanCodeFenceContent = (block) => block
    .replace(/^```[^\n]*\n?/, "")
    .replace(/```$/, "")
    .trim();
const extractCodeFenceBlocks = (raw) => {
    const blocks = [];
...
const extractBraceDelimitedBlocks = (raw) => {
    const blocks = [];
    let depth = 0;
    let start = -1;
    let inString = false;
...
const extractJsonObjectFromIndex = (raw, startIndex) => {
    let depth = 0;
    let inString = false;
    let escaping = false;
...
const extractJsonArrayFromIndex = (raw, startIndex) => {
    let depth = 0;
    let inString = false;
    let escaping = false;
    for (let index = startIndex; index < raw.length; index += 1) {
...
const extractStringOrNull = (raw, key) => {
...
const extractReActJSON = (raw) => {
    ...
    for (const block of braceBlocks) {
        ...
        const keyCount = EXPECTED_REACT_KEYS.reduce((count, key) => count + (parsed[key] !== undefined && parsed[key] !== null ? 1 : 0), 0);
    ...
    if (actionInputMatch) {
        if (actionInputMatch[1].toLowerCase() === "null") {
            ...
            const objectStart = actionInputMatch.index + actionInputMatch[0].length - 1;
...
const normalizeReActString = (value) => {
...
const validateReActResponse = (extracted) => {
    ...
    const rawActionInput = extracted.actionInput !== undefined
    ...
    if (extracted.actions !== undefined) {
        if (Array.isArray(extracted.actions) && extracted.actions.length > 0) {
            ...
            for (const entry of extracted.actions) {
                ...
                const rawAct = normalizeReActString(entry.action);
                let rawInput = entry.actionInput;
                if (rawInput !== null &&
                    rawInput !== undefined &&
            ...
            if (!malformed) {
                ...
                if (Array.isArray(extracted.actions) && extracted.actions.length > 0) {
                    ...
                    const firstAct = normalizeReActString(first.action);
                    let firstInput = first.actionInput;
                    if (firstInput !== null &&
...
const buildAllowedToolSet = (rawAllowedTools, registry) => {
    const allowed = new Map();
    const unknownTools = [];
    const toolCandidates = Array.isArray(rawAllowedTools)
...
const validateToolCall = (action, allowed, context) => {
...
const createLoopGuard = (config = {}) => {
    const maxConsecutiveParseFailures = Number.isFinite(Number(config.maxConsecutiveParseFailures ?? 3))
        ? Number(config.maxConsecutiveParseFailures)
        : 3;
    const maxConsecutiveToolFailures = Number.isFinite(Number(config.maxConsecutiveToolFailures ?? 3))
        ? Number(config.maxConsecutiveToolFailures)
        : 3;
    const maxRepeatedActions = Number.isFinite(Number(config.maxRepeatedActions ?? 3))
    ...
    let totalFailures = 0;
    const perToolFailures = {};
    let deniedActionCount = 0;
    const recentActions = [];
    const recordAction = (action, fromParallel = false) => {
        if (!action)
            return;
    ...
    return {
        ...
        recordToolSuccess: (_toolType) => {
            consecutiveToolFailures = 0;
        },
        recordAction: (action, fromParallel = false) => {
            recordAction(String(action ?? "").trim(), fromParallel);
        },
        recordDeniedAction: (_action) => {
            deniedActionCount += 1;
        },
        shouldStop: () => {
            ...
            for (const count of Object.values(actionCounts)) {
                if (count >= 5) {
                    return {
                        stop: true,
                        reason: "The agent is stuck in a repetitive loop calling the same action repeatedly without progress.",
                    };
                }
            }
            if (recentActions.length >= 6) {
                const window = recentActions.slice(-6);
                const pairToString = (index) => `${window[index]?.action ?? ""}|${window[index + 1]?.action ?? ""}`;
                ...
                if (first && first === second && second === third) {
                    return {
                        stop: true,
                        reason: "The agent is cycling through the same two-step sequence repeatedly without making progress.",
                    };
                }
            }
            if (recentActions.length >= 3) {
                ...
                for (const counts of Object.values(countsByAction)) {
                    if (counts.parallelCount >= 3) {
                        return {
                            stop: true,
                            reason: "The agent is repeatedly including the same tool in parallel batches without using its results.",
                        };
                    }
                }
            }
            if (totalFailures >= maxTotalFailures) {
                return {
                    stop: true,
                    reason: "The agent has exceeded its total failure budget across parse errors and tool failures.",
                };
            }
            if (deniedActionCount >= 3) {
                return {
                    stop: true,
                    reason: "The agent repeatedly attempted unauthorized tool calls and cannot operate within its permissions.",
                };
            }
...
const normalizeFieldType = (value) => {
    ...
    if (asString.includes("string"))
        return "string";
    if (asString.includes("number") ||
        asString.includes("int") ||
        asString.includes("float") ||
        asString.includes("double"))
        return "number";
    if (asString.includes("bool"))
        return "boolean";
...
const inferToolSchema = (toolDefinition) => {
    if (!isObject(toolDefinition)) {
        return { fields: [] };
    }
    const extractDescriptors = (source) => {
        if (!isObject(source)) {
            return [];
        }
        ...
        if (isObject(source.properties)) {
            const requiredNames = Array.isArray(source.required)
                ? source.required.map((name) => String(name))
                : [];
            return Object.entries(source.properties).map(([name, property]) => ({
                name,
                type: normalizeFieldType(property?.type || property?.schema?.type || property?.dataType),
    ...
    const validationSource = toolDefinition.validation || {};
    const candidates = [
...
const validateToolInput = (actionInput, schema, toolType) => {
    const warnings = [];
    const errors = [];
    ...
    const coerceValue = (value, expectedType, fieldName) => {
        if (value === null || value === undefined) {
            return value;
        }
        if (expectedType === "string") {
            if (typeof value === "number" || typeof value === "boolean") {
                warnings.push(`${fieldName} was coerced from ${typeof value} to string`);
                return String(value);
            }
            if (typeof value === "string") {
                return value;
    ...
    let normalizedInput = actionInput;
    if (typeof actionInput === "string") {
        const trimmed = actionInput.trim();
        try {
            const parsed = JSON.parse(trimmed);
            normalizedInput = parsed;
            warnings.push(`actionInput was a JSON string and was parsed automatically`);
        }
        catch {
            const requiredStringFields = fields.filter((field) => field.required && field.type === "string");
            if (requiredStringFields.length === 1) {
                normalizedInput = {
                    [requiredStringFields[0].name]: actionInput,
                };
                warnings.push(`actionInput was a raw string and was mapped to field ${requiredStringFields[0].name}`);
            }
            else {
                return {
                    valid: false,
    ...
    const sanitizedKnown = {};
    const inputKeys = Object.keys(normalizedInput);
    for (const field of fields) {
        const rawValue = normalizedInput[field.name];
        const hasKey = Object.prototype.hasOwnProperty.call(normalizedInput, field.name);
        if (!hasKey) {
            if (field.required) {
                errors.push(`${field.name} is required but was not provided for ${toolType}`);
            }
            continue;
        }
        if (typeof rawValue === "function") {
            errors.push(`${field.name} contained a function value which was removed for security`);
            continue;
        }
        ...
        if (field.type === "string") {
            if (typeof rawValue === "number" || typeof rawValue === "boolean") {
                ...
                errors.push(`${field.name} expected string but received ${typeOfValue(rawValue)}`);
                continue;
            }
        }
            ...
            if (typeof rawValue === "string") {
                ...
                errors.push(`${field.name} expected number but received ${typeOfValue(rawValue)}`);
                continue;
            }
        }
            ...
            if (typeof rawValue === "string") {
                ...
                errors.push(`${field.name} expected boolean but received ${typeOfValue(rawValue)}`);
                continue;
            }
        }
        else if (field.type === "object") {
            if (!isPlainObject(rawValue)) {
                errors.push(`${field.name} expected object but received ${typeOfValue(rawValue)}`);
                continue;
            }
        }
        else if (field.type === "array") {
            if (!Array.isArray(rawValue)) {
                errors.push(`${field.name} expected array but received ${typeOfValue(rawValue)}`);
                continue;
            }
        }
        sanitizedKnown[field.name] = coercedValue;
    }
    const sanitizedOutput = { ...sanitizedKnown };
    ...
    if (errors.length === 0) {
        for (const key of inputKeys) {
            ...
            if (typeof value === "function") {
                errors.push(`${key} contained a function value which was removed for security`);
                continue;
            }
            sanitizedOutput[key] = value;
            warnings.push(`${key} is not in the schema and was passed through unvalidated`);
        }
    }
    if (errors.length > 0) {
        return {
...
const normalizeToolOutput = (rawToolResult) => {
    try {
        ...
        if (isObject(rawToolResult) && "output" in rawToolResult) {
            ...
            if (rawToolResult.success === false && rawToolResult.error) {
                const errorMsg = typeof rawToolResult.error === "string"
                    ? rawToolResult.error
...
const summarizeToolOutput = (normalizedOutput, toolType, iteration, taskDescription) => {
    try {
        ...
        const maybeSentenceSplit = (input) => {
        ...
        const compressWebSearch = (input) => {
            ...
            for (const block of blocks) {
                ...
                if (title || url || snippet) {
                    ...
                    results.push(`${title ? `Title: ${title}` : ""}${title && url ? "; " : ""}${url ? `URL: ${url}` : ""}${snippetSentence ? `; Snippet: ${snippetSentence}` : ""}`.replace(/; $/, ""));
                }
            }
        ...
        const compressHttpRequest = (input) => {
            const statusMatch = /(?:status|statusCode|status_code)\s*[:=]\s*(\d{3})/i.exec(input);
            ...
            if (isObject(jsonBody)) {
                ...
                if (topKeys.length > 0) {
                    parts.push(`top-level keys: ${topKeys
                        .map((key) => {
        ...
        const cleanSentence = (sentence) => sentence.replace(/\s+/g, " ").trim();
        const isSignalWord = (value) => {
            const signalWords = [
                "result",
                "found",
                "error",
            ...
            return signalWords.some((word) => new RegExp(`\\b${word}\\b`, "i").test(value));
        };
        ...
        const finalText = compressed.trim() ||
...
const buildContextEntry = (toolType, iteration, thought, action, summarizedOutput, maxIterations, parallelCount) => {
    const safeThought = String(thought ?? "").trim();
    const safeAction = String(action ?? "").trim();
    const safeOutput = String(summarizedOutput ?? "").trim() || "[no summary available]";
    const iterLabelBase = `[Iter ${iteration}/${Number(maxIterations ?? 0)}]`;
    const parallelLabel = parallelCount && parallelCount > 1
...
const buildSystemPrompt = (userSystemPrompt, toolDescriptions, taskInput, maxIterations, strictMode = true) => {
    ...
    const numberedToolLines = sortedTools.length > 0
        ? sortedTools
            .map((t, i) => `${i + 1}. ${String(t.type)} — ${String(t.description || "").trim()}`)
            .join("\n")
 ...
 */
const executeParallelTools = async (actions, allowed, context, validateToolCallFn, validateToolInputFn, normalizeToolOutputFn, summarizeToolOutputFn, loopGuardObj) => {
    const results = [];
    ...
    for (const entry of actions) {
        ...
        const normalized = callValidation.normalizedAction ||
            String(rawAction ?? "")
                .trim()
                .toLowerCase();
        ...
        const inputValidation = validateToolInputFn(entry.actionInput, toolSchema, normalized);
    ...
    const execPromises = results.map(async (r, idx) => {
        if (!r.valid)
            return { idx, fulfilled: false, error: r.error };
    ...
    for (let i = 0; i < settled.length; i++) {
        ...
        if (s.status === "fulfilled") {
            ...
            const summarized = summarizeToolOutputFn(toolText, toolType, 0, context?.input?.text || context?.config?.task);
            finalResults.push({
                toolType,
                sanitizedInput: results[payload.idx]?.sanitizedInput || {},
...
const mergeParallelResults = (results, thought, iteration, maxIterations) => {
    ...
    if (failed.length === results.length) {
        footerParts.push("All parallel tool calls failed; consider a different approach.");
    }
    else if (failed.length > 0) {
        footerParts.push(`Some tools failed: ${Array.from(new Set(failed)).join(", ")}.`);
    }
    ...
    const truncatedLines = lines.map((ln) => ln.length > per ? ln.slice(0, per - 3) + "..." : ln);
...
const reactAgentHandler = async (context) => {
    ...
    const taskInput = String(config.task ||
        config.prompt ||
        context.input?.text ||
    ...
    const { allowed: allowedToolMap, unknownTools } = buildAllowedToolSet(config.allowedTools, registry);
    if (!apiKey) {
        return {
            success: false,
    ...
    const scratchpad = [];
    let history = [
        buildSystemPrompt(systemPrompt, toolDescriptions, taskInput, maxIterations, true),
    ];
    if (unknownTools.length > 0) {
        history.push(`Configured tool types were not found in the injected registry: ${unknownTools.join(", ")}. These tools will be ignored.`);
    }
    const trimHistory = () => {
        if (history.length > 6) {
    ...
    const loopGuard = createLoopGuard({
        maxConsecutiveParseFailures: parseThreshold(config.maxConsecutiveParseFailures ??
            config["maxConsecutiveParseFailures"], 3),
        maxConsecutiveToolFailures: parseThreshold(config.maxConsecutiveToolFailures ?? config["maxConsecutiveToolFailures"], 3),
        maxRepeatedActions: parseThreshold(config.maxRepeatedActions ?? config["maxRepeatedActions"], 3),
        maxTotalFailures: parseThreshold(config.maxTotalFailures ?? config["maxTotalFailures"], 5),
    });
    let iteration = 0;
    let lastRawResponse = "";
    ...
    while (iteration < maxIterations) {
        ...
        if ((iteration + 1) % 3 === 0) {
            const formatLine = "OUTPUT FORMAT: JSON object with keys: thought, action, actionInput, finalAnswer. No text outside the JSON object.";
        ...
        if (estimateTokens(currentHistoryText) > 6000) {
            ...
            scratchpad.push("[warning] Context was compressed due to token budget pressure.");
        }
        ...
        try {
            ...
            const invokeTimeoutMs = provider === "gemini" ? Math.max(baseTimeoutMs, 120000) : baseTimeoutMs;
            let res = null;
            for (let attempt = 0; attempt <= maxLLMRetries; attempt++) {
                try {
                    res = await llm.invoke([new messages_1.HumanMessage(prompt)], {
                    ...
                    const rateLimited = /429|Too Many Requests|Too Many Req|rate limit|RateLimit/i.test(errStr);
                    const timedOut = /LLM invoke timeout|timed out|timeout|ETIMEDOUT|ESOCKETTIMEDOUT/i.test(errStr);
                    const serverError = /5\d\d|Service Unavailable|Gateway Timeout|Bad Gateway/i.test(errStr);
                    const shouldRetry = (rateLimited || timedOut || serverError) && attempt < maxLLMRetries;
            ...
            if (!extracted) {
                ...
                history.push("Previous response could not be parsed. Please respond with only a JSON object containing the four expected keys: thought, action, actionInput, finalAnswer.");
                trimHistory();
                const guardCheck = loopGuard.shouldStop();
                if (guardCheck.stop) {
                    guardTriggered = true;
            ...
            if (parsed.parallelActions &&
                ...
                try {
                    ...
                    const parallelResults = await executeParallelTools(parallelActions, allowedToolMap, context, validateToolCall, validateToolInput, normalizeToolOutput, summarizeToolOutput, loopGuard);
                    const merged = mergeParallelResults(parallelResults, thought ?? "", iteration, maxIterations);
                    history.push(merged);
                    trimHistory();
                    ...
                    scratchpad.push(`[${iteration}] parallel execution error=${String(err?.message || err)}`);
                }
                // Continue to next iteration after handling parallel batch
                continue;
            }
            else if (parsed.parallelActions &&
                Array.isArray(parsed.parallelActions) &&
                parsed.parallelActions.length === 1) {
            ...
            if (typeof action === "string" &&
                ...
                history.push("Tool react-agent is not allowed within react-agent node.");
                trimHistory();
            ...
            if (action) {
                ...
                if (!validation.permitted) {
                    ...
                    trimHistory();
                    scratchpad.push(`[${iteration}] denied action=${deniedAction} reason=${validation.reason}`);
                    if (typeof context.emitEvent === "function") {
                        try {
                            context.emitEvent({
                                type: "tool-denied",
                ...
                const toolSchema = inferToolSchema(nodeDef);
                const inputValidation = validateToolInput(actionInput, toolSchema, validation.normalizedAction);
                if (!inputValidation.valid) {
                    const schemaDescription = toolSchema.fields.length
                        ? `Expected fields: ${toolSchema.fields
                            .map((field) => `${field.name}${field.required ? " (required)" : ""}:${field.type}`)
                            .join(", ")}`
                        : "No input schema available.";
                    history.push(`Tool call to ${validation.normalizedAction} was rejected due to input validation errors: ${inputValidation.errors.join("; ")}. ${schemaDescription}`);
                    trimHistory();
                    scratchpad.push(`[${iteration}] validation ${validation.normalizedAction} passed=false errors=${inputValidation.errors.length} warnings=${inputValidation.warnings.length} fields=${Object.keys(inputValidation.sanitized).join(", ") || "none"}`);
                    continue;
                }
                if (inputValidation.warnings.length > 0) {
                    scratchpad.push(`[${iteration}] validation warnings for ${validation.normalizedAction}: ${inputValidation.warnings.join("; ")}`);
                }
                scratchpad.push(`[${iteration}] validation ${validation.normalizedAction} passed=true errors=0 warnings=${inputValidation.warnings.length} fields=${Object.keys(inputValidation.sanitized).join(", ") || "none"}`);
                // Execute the validated tool handler from the allow-list
                try {
                    if (!nodeDef || typeof nodeDef.handler !== "function") {
                        history.push(`Tool ${validation.normalizedAction} not found or has no handler`);
                        trimHistory();
                    ...
                    try {
                        ...
                        if (serializedTool.length > 50000) {
                            scratchpad.push("[warning] Tool returned an unusually large response; summarizing output for prompt safety.");
                            let outputSummary = "";
                            try {
                        ...
                        scratchpad.push("[warning] Tool returned an unusually large or non-serializable response; summarizing output for prompt safety.");
                        let outputSummary = "";
                        try {
                            const outputValue = toolResult?.output;
                            outputSummary =
                    ...
                    const summarizedToolText = summarizeToolOutput(toolText, validation.normalizedAction, iteration, taskInput);
                    history.push(buildContextEntry(validation.normalizedAction, iteration, thought ?? "", action ?? "", summarizedToolText, maxIterations));
                    trimHistory();
                    scratchpad.push(`[${iteration}] action=${action} output=${toolText}`);
                }
                catch (err) {
                    loopGuard.recordToolFailure(validation.normalizedAction);
                    history.push(`Tool ${action} execution error: ${String(err?.message || err)}`);
                    trimHistory();
                    scratchpad.push(`[${iteration}] action=${action} error=${String(err?.message || err)}`);
                }
...
exports.reactAgentHandler = reactAgentHandler;
exports.default = exports.reactAgentHandler;
