...
if (REDIS_URL) {
  ...
  redisClient.on("ready", () => {
    if (!redisConnected) {
      console.info(
        `Redis queue reconnected after ${redisReconnectAttempt} attempt${redisReconnectAttempt === 1 ? "" : "s"}`,
      );
    } else {
...
class DatabaseQueue {
  ...
  async startProcessing() {
    ...
    console.info("DB queue worker is alive and ready to fetch jobs");
...
exports.agentRunSchema = zod_1.z
  .object({
    ...
    nodes: zod_1.z
      ...
      .min(1),
    edges: zod_1.z
      .array(validation_1.AgentEdgeInputSchema)
      .optional()
      .default([]),
...
async function processJobFunction(jobData, jobId) {
  ...
  console.info("Processing queued job", {
    jobId,
    executionId,
    userId,
    agentId,
  ...
  try {
    ...
    await supabaseClient_1.supabase
      ...
      .eq("id", jobId);
    const decryptedApiKeys = JSON.parse(
      (0, encryption_1.decryptValue)(apiKeys),
    );
    const result = await (0, agentEngine_1.executeWorkflow)(
      config?.nodes || [],
      config?.edges || [],
      input ?? {},
      decryptedApiKeys,
