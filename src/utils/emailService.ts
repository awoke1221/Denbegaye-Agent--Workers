...
async function sendViaMailgun(options: EmailOptions): Promise<EmailResult> {
  try {
    ...
    if (!apiKey || !domain) {
      throw new Error(
        "Mailgun credentials not configured (MAILGUN_API_KEY and MAILGUN_DOMAIN required)",
      );
    }
    ...
    logger.info(`Email sent successfully via Mailgun to ${options.to}`);
...
export async function sendEmailBatch(
  ...
  try {
    ...
    logger.error("Batch email send failed:", error);
    return [
      {
        success: false,
        error: error instanceof Error ? error.message : String(error),
