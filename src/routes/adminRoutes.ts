...
const getQueueDashboardStats = async () => {
  ...
  const [upcomingRetriesResult, deadLetterJobsResult, recentFailuresResult] =
    await Promise.all([
      supabase
        .from("job_queue")
        .select(
          "id, job_type, status, attempt_count, max_attempts, scheduled_at, next_retry_at, error_message, payload, created_at, updated_at",
        )
        .eq("status", "queued")
        .gt("next_retry_at", nowIso)
      ...
      supabase
        .from("job_queue")
        .select(
          "id, job_type, status, attempt_count, max_attempts, error_message, payload, created_at, updated_at",
        )
        .eq("status", "dead_letter")
  ...
  return {
    ...
    backoff: {
      ...
      maxDelayMs: 300000,
      description:
        "Retries are scheduled with exponential backoff: delay = min(baseDelayMs * 2^attempt_count, maxDelayMs)",
    },
...
export const setupAdminRoutes = (app: Express) => {
  ...
  app.get(
    ...
    async (req: AdminRequest, res: Response) => {
      try {
        ...
        const limit = parseIntOrDefault(req.query.limit as string, 50);
        ...
        let query = supabase
          .from("user_agents")
          .select(
            "id, name, description, status, user_id, created_at, updated_at, version",
            {
              count: "exact",
            },
          );
        ...
        const ownerIds = Array.from(
          new Set((agents || []).map((agent) => agent.user_id)),
        );
        ...
        const statsByAgent = new Map<string, any>();
  ...
  app.get(
    ...
    async (req: AdminRequest, res: Response) => {
      try {
        const statusValues = parseIdList(req.query.status as string);
        const limit = parseIntOrDefault(req.query.limit as string, 50);
        ...
        const userIds = Array.from(
          new Set((executions || []).map((exec) => exec.user_id)),
        );
        const agentIds = Array.from(
          new Set((executions || []).map((exec) => exec.agent_id)),
        );
        ...
        const [{ data: users }, { data: agents }] = await Promise.all([
          userIds.length > 0
            ...
            : { data: [] },
          agentIds.length > 0
            ? supabase.from("user_agents").select("id, name").in("id", agentIds)
            : { data: [] },
        ]);
        ...
        const resultExecutions = (executions || []).map((execution) => {
          ...
          return {
            ...
            agent_name:
              agentMap.get(execution.agent_id)?.name || "Unknown Agent",
  ...
  app.patch(
    ...
    async (req: AdminRequest, res: Response) => {
      try {
        const { executionId, action } = req.body as {
          executionId: string;
          action: string;
        };
  ...
  app.get(
    ...
    async (req: AdminRequest, res: Response) => {
      try {
          ...
          (subscriptionRows || []).length > 0
            ? await supabase
                ...
                .in(
                  "id",
                  Array.from(
                    new Set((subscriptionRows || []).map((sub) => sub.plan_id)),
                  ),
                )
            : { data: [] };

        const planMap = new Map(
          (plans.data || []).map((plan: any) => [plan.id, plan]),
        );
        ...
        const executionCounts = (executionRows || []).reduce(
          (acc: Record<string, number>, row: any) => {
            acc[row.user_id] = (acc[row.user_id] || 0) + 1;
            return acc;
          },
        ...
        const subscriptionMap = new Map(
          (subscriptionRows || []).map((sub: any) => [sub.user_id, sub]),
        );
          ...
          stats: {
            ...
            subscription: subscriptionMap.has(profile.id)
              ? {
                  ...
                  plan:
                    planMap.get(subscriptionMap.get(profile.id).plan_id) ||
                    null,
                }
              : null,
          },
  ...
  app.patch(
    ...
    async (req: AdminRequest, res: Response) => {
      try {
        const { subscriptionId, action, status, limits } = req.body as {
    ...
    async (req: AdminRequest, res: Response) => {
      try {
        const { data: apiKeys, error } = await supabase
          .from("user_api_keys")
          .select(
            "id, user_id, provider, label, is_active, created_at, last_used_at",
          );
        if (error) {
          throw error;
        }
  ...
  app.put(
    ...
    async (req: AdminRequest, res: Response) => {
      try {
        const updated = req.body as Partial<typeof systemSettings>;
