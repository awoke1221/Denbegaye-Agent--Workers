...
import { adminOnly } from "./adminRoutes";
...
const validateBlogPostPayload = (payload: any) => {
  const errors: string[] = [];
  ...
  if (
    ...
    payload.tags.some((tag: any) => typeof tag !== "string")
  ) {
...
router.get("/blogs", adminOnly, async (_req: Request, res: Response) => {
  try {
...
router.post("/blogs", adminOnly, async (req: Request, res: Response) => {
  try {
...
router.put("/blogs/:slug", adminOnly, async (req: Request, res: Response) => {
  try {
...
export const setupBlogAdminRoutes = (app: Express) => {
