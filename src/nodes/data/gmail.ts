...
const gmailInputSchema = z.object({
  ...
  messageId: z.union([z.string(), z.array(z.string())]).optional(),
  query: z.string().optional(),
  labelIds: z.union([z.array(z.string()), z.string()]).optional(),
  addLabelIds: z.union([z.array(z.string()), z.string()]).optional(),
  removeLabelIds: z.union([z.array(z.string()), z.string()]).optional(),
  ...
  apiHeaders: z.union([z.record(z.string()), z.string()]).optional(),
  apiParams: z.union([z.record(z.any()), z.string()]).optional(),
  apiBody: z.union([z.any(), z.string()]).optional(),
});

function createOAuth2Client(config: z.infer<typeof gmailConfigSchema>) {
...
function normalizeAttachments(
  value:
    ...
    | string
    | undefined,
): Array<{ filename: string; mimeType: string; content: string }> | undefined {
  if (!value) {
    return undefined;
  }
  if (Array.isArray(value)) {
...
async function downloadAttachment(
  ...
  if (downloadPath) {
    ...
    const contentDisposition = message.data.payload?.headers?.find(
...
function normalizeApiBody(value: any): any {
  if (typeof value === "string") {
    try {
...
function buildRawMessage(options: {
  ...
  if (options.threadId) headers.push(`Thread-Id: ${options.threadId}`);
  ...
  if (htmlBody && htmlBody !== textBody) {
    headers.push(
      `Content-Type: multipart/alternative; boundary="ALT-${Date.now()}"`,
    );
...
function extractAttachments(message: any): any[] {
  ...
  function collect(partsList: any[]) {
...
const gmailNode: NodeDefinition = {
  id: "gmail",
  type: "data-gmail",
  name: "Google Gmail",
  description:
    "Manage Gmail messages with send, reply, search, draft, labels, attachments, and API calls",
  category: "data",
  icon: "✉️",
  color: "#EA4335",
  ...
  inputs: [
    {
      ...
      required: true,
      description:
        "Action to perform: send, reply, search, get, copy, move, updateLabels, markAsRead, markAsUnread, delete, createDraft, sendDraft, listAttachments, apiCall",
    },
    {
      id: "to",
      label: "To",
    ...
    },
    {
      ...
      type: "string",
      description:
        "Message ID for get, modify, delete, and attachments operations",
    },
    {
      id: "query",
      label: "Search Query",
  ...
  handler: async (
    ...
    try {
      ...
      const input = {
        ...
        apiParams,
        apiBody,
      } as z.infer<typeof gmailInputSchema>;
      ...
      const gmailMessages = gmail.users.messages as any;
      ...
      switch (input.action) {
        ...
        case "reply": {
          ...
          const originalSubject =
            parseHeaderValue(originalHeaders, "Subject") || "";
          const originalFrom = parseHeaderValue(originalHeaders, "From") || "";
          const originalMessageId = parseHeaderValue(
            originalHeaders,
            "Message-ID",
          );
        ...
        case "createDraft": {
          if (!normalizedTo) {
            throw new Error(
              "Recipient address is required for createDraft action",
            );
          }
          if (!input.body && !input.htmlBody) {
            throw new Error(
              "Body or HTML body is required for createDraft action",
            );
          }
          const raw = buildRawMessage({
            from: fromAddress,
        ...
        case "listAttachments": {
          if (!normalizedMessageId) {
            throw new Error(
              "Message ID is required for listAttachments action",
            );
          }
        ...
        case "downloadAttachment": {
          if (!normalizedMessageId || !normalizedAttachmentId) {
            throw new Error(
              "Message ID and attachment ID are required for downloadAttachment action",
            );
          }
          ...
          for (let i = 0; i < recipients.length; i += batchSize) {
            ...
            batchResults.forEach((result, index) => {
              if (result.status === "fulfilled") {
                ...
                logs.push(
                  `Failed to send to ${batch[index]}: ${result.reason}`,
                );
              }
            });
          }
        ...
        case "batchDelete": {
          ...
          const batchPromises = messageIds.map((messageId) =>
            gmail.users.messages.delete({ userId, id: messageId }),
          );
        ...
        case "replyToThread": {
          if (!normalizedThreadId || !normalizedMessageId) {
            throw new Error(
              "Thread ID and message ID are required for replyToThread action",
            );
          }
        ...
        case "createFilter": {
          if (!input.filterCriteria || !input.filterAction) {
            throw new Error(
              "Filter criteria and action are required for createFilter action",
            );
          }
