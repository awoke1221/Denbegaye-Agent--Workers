...
const googleSheetsConfigSchema = z.object({
  authMethod: z.enum(["manual", "google-oauth"]).default("manual"),
  // Manual auth fields
...
const googleSheetsTriggerConfigSchema = z.object({
  authMethod: z.enum(["manual", "google-oauth"]).default("manual"),
  // Manual auth fields
  accessToken: z.string().optional(),
  refreshToken: z.string().optional(),
  ...
  triggerType: z.enum(["onNewRow", "onRowChanged", "onSheetChanged"]),
  checkInterval: z.number().min(1000).default(60000), // 60 seconds
  useHeaderRow: z.boolean().default(true),
});

// Helper function to create OAuth2 client
function createOAuth2Client(config: z.infer<typeof googleSheetsConfigSchema>) {
  if (config.authMethod === "google-oauth") {
    if (!config.providerToken) {
      throw new Error(
        "Provider token required for Google OAuth authentication",
      );
    }
    ...
    if (!config.accessToken || !config.clientId || !config.clientSecret) {
      throw new Error(
        "Access token, client ID, and client secret required for manual authentication",
      );
    }
    const oauth2Client = new google.auth.OAuth2(
      config.clientId,
...
function getSpreadsheetId(
  input: any,
  config: z.infer<typeof googleSheetsConfigSchema>,
): string {
...
function convertDataToArrays(data: any, headers?: string[]): any[][] {
...
function convertArraysToObjects(
  data: any[][],
  headers?: string[],
  ...
  return data.map((row, index) => {
    ...
    if (includeRowNumbers) obj._rowNumber = startRow + index + 1;
    return obj;
  });
}
...
function parseGoogleError(error: any): string {
...
): Promise<{ data: any[]; rowCount: number; isEmpty: boolean }> {
...
async function handleAppend(
  ...
  headers?: string[],
): Promise<{ updatedRange: string; updatedRows: number; rowCount: number }> {
...
async function handleDelete(
  ...
  sheetId: number,
  rowIndexes: number[],
): Promise<{ affectedRows: number; rowCount: number }> {
...
async function handleClear(
  ...
  spreadsheetId: string,
  range: string,
): Promise<{ clearedRange: string; rowCount: number }> {
  ...
  dataRows.forEach((row: any[], index: number) => {
    if (row[columnIndex] == value) {
      ...
      if (headers) {
        const obj: any = {};
        headers.forEach(
          (header: string, i: number) => (obj[header] = row[i] || ""),
        );
        if (includeRowNumbers) (obj as any)._rowNumber = startRow + index + 1;
...
async function handleDeleteSheet(
  ...
  sheetName?: string,
  sheetId?: number,
): Promise<{ deletedSheetId: number; deletedSheetName: string }> {
  ...
  if (!sheetId) {
    ...
    sheetId = sheet.properties?.sheetId;
    if (!sheetId) throw new Error(`Sheet "${sheetName}" has no sheetId`);
  }
 ...
 */
async function handleListSpreadsheets(
  authClient: any,
  query?: string,
): Promise<{ spreadsheets: Array<{ id: string; name: string }> }> {
...
async function handleBatchUpdate(
  ...
  valueInputOption: string,
): Promise<{ updatedRanges: string[]; totalUpdatedRows: number }> {
  ...
  const updatedRanges =
    response.data.responses?.map((res: any) => res.updatedRange || "") || [];
...
const googleSheetsNode: NodeDefinition = {
  ...
  inputs: [
    {
      ...
      required: true,
      description:
        "Action to perform (read, append, update, delete, clear, find, upsert, createSpreadsheet, createSheet, deleteSheet, getMetadata, batchUpdate, formatCells, bulkDeleteRows)",
    },
    {
      id: "spreadsheetId",
    ...
    },
    {
      ...
      required: false,
      description:
        "Optional search text to filter spreadsheet names when listing accessible sheets",
    },
    {
      id: "range",
      label: "Range",
  ...
  outputs: [
    ...
    },
    {
      ...
      type: "array",
      description:
        "List of accessible spreadsheets for the connected Google account",
    },
    {
      id: "updatedRanges",
  ...
  validation: {
    input: z.object({
      ...
      useHeaderRow: z.boolean().default(true),
      includeRowNumbers: z.boolean().default(false),
      ...
      updates: z
        .array(
          z.object({ range: z.string(), values: z.array(z.array(z.any())) }),
        )
        .optional(),
      formatting: z
        .object({
          ...
          backgroundColor: z
            .object({ red: z.number(), green: z.number(), blue: z.number() })
            .optional(),
          textColor: z
            .object({ red: z.number(), green: z.number(), blue: z.number() })
            .optional(),
        ...
        .optional(),
      rowIndexes: z.array(z.number()).optional(),
    }),
    output: z.object({
      data: z.any(),
      rowCount: z.number(),
      ...
      spreadsheets: z.array(z.any()).optional(),
      updatedRanges: z.array(z.string()).optional(),
      totalUpdatedRows: z.number().optional(),
      formattedRange: z.string().optional(),
    ...
    try {
      ...
      switch (input.action) {
        case "read": {
          ...
          const { data, rowCount, isEmpty } = await handleRead(
            sheets,
            spreadsheetId,
            range,
            input.useHeaderRow,
            input.includeRowNumbers,
          );
        ...
        case "append": {
          ...
          const range = buildRange(input.sheetName || "", input.range);
          if (!input.data) throw new Error("Data required for append");
          ...
          const { updatedRange, updatedRows, rowCount } = await handleAppend(
            sheets,
            spreadsheetId,
            range,
            input.data,
        ...
        case "update": {
          ...
          const { updatedRange, updatedRows, rowCount } = await handleUpdate(
            sheets,
            spreadsheetId,
            range,
            input.data,
        ...
        case "delete": {
          ...
          if (!sheetName) throw new Error("Sheet name required for delete");
          ...
          const { affectedRows, rowCount } = await handleDelete(
            sheets,
            spreadsheetId,
            sheetId,
            input.rowIndexes,
        ...
        case "clear": {
          ...
          const { clearedRange, rowCount } = await handleClear(
            sheets,
            spreadsheetId,
            range,
          );
        ...
        case "find": {
          ...
          const { data, rowCount, isEmpty } = await handleFind(
            sheets,
            spreadsheetId,
            range,
            input.column,
        ...
        case "upsert": {
          ...
          const { action, updatedRange, updatedRows, rowCount } =
            await handleUpsert(
              sheets,
              spreadsheetId,
              range,
        ...
        case "createSpreadsheet": {
          ...
          const { spreadsheetId, spreadsheetUrl } =
            await handleCreateSpreadsheet(sheets, input.title);
        ...
        case "createSheet": {
          ...
          const { sheetId, sheetName } = await handleCreateSheet(
            sheets,
            spreadsheetId,
            input.sheetName,
          );
        ...
        case "deleteSheet": {
          ...
          const { deletedSheetId, deletedSheetName } = await handleDeleteSheet(
            sheets,
            spreadsheetId,
            input.sheetName,
            input.sheetId,
        ...
        case "getMetadata": {
          ...
          const { title, sheets: sheetsList } = await handleGetMetadata(
            sheets,
            spreadsheetId,
          );
        ...
        case "listSpreadsheets": {
          const { spreadsheets } = await handleListSpreadsheets(
            oauth2Client,
            input.query,
          );
          result = { spreadsheets };
        ...
        case "batchUpdate": {
          ...
          const { updatedRanges, totalUpdatedRows } = await handleBatchUpdate(
            sheets,
            spreadsheetId,
            input.updates,
            input.valueInputOption,
        ...
        case "formatCells": {
          ...
          const { formattedRange } = await handleFormatCells(
            sheets,
            spreadsheetId,
            range,
            input.formatting,
        ...
        case "bulkDeleteRows": {
          ...
          const { affectedRows, rowCount } = await handleBulkDeleteRows(
            sheets,
            spreadsheetId,
            sheetId,
            input.rowIndexes,
...
const googleSheetsTriggerNode: NodeDefinition = {
  ...
  handler: async (
    ...
    try {
      ...
      const triggerState = (context as any).triggerState || {};
      ...
      let newRows: any[] = [];
      let changedRows: Array<{ rowNumber: number; before: any; after: any }> =
        [];
      ...
      if (config.triggerType === "onNewRow") {
        if (currentRowCount > previousRowCount) {
          ...
          newRows = convertArraysToObjects(
            currentValues.slice(previousRowCount),
            config.useHeaderRow ? currentValues[0] : undefined,
            false,
            previousRowCount + startRow,
          );
...
export { googleSheetsNode, googleSheetsTriggerNode };
