...
const gmailTriggerConfigSchema = z.object({
  ...
  checkInterval: z.number().min(1000).default(30000), // 30 seconds minimum
  includeBody: z.boolean().default(true),
  markAsRead: z.boolean().default(true),
  triggerType: z.enum(["polling", "webhook"]).default("polling"),
  ...
  searchFilters: z
    .object({
      ...
      excludeLabelIds: z.array(z.string()).optional(),
      dateAfter: z.string().optional(), // ISO date
      dateBefore: z.string().optional(), // ISO date
      sizeLarger: z.number().optional(), // bytes
      sizeSmaller: z.number().optional(), // bytes
    })
    .optional(),
...
function createOAuth2Client(config: z.infer<typeof gmailTriggerConfigSchema>) {
...
function extractAttachments(message: any): any[] {
  ...
  function walkParts(partsList: any[]) {
...
function buildAdvancedQuery(
  config: z.infer<typeof gmailTriggerConfigSchema>,
): string {
  const filters = config.searchFilters;
  if (!filters) return config.query;
  ...
  if (filters.dateBefore) {
    ...
    queryParts.push(
      `before:${date.getFullYear()}/${date.getMonth() + 1}/${date.getDate()}`,
    );
  }
...
const gmailTriggerNode: NodeDefinition = {
  id: "gmail-trigger",
  type: "trigger-gmail",
  name: "Gmail Trigger",
  description:
    "Trigger workflow execution when new Gmail messages match a query",
  category: "trigger",
  icon: "📨",
  color: "#D14836",
  ...
  handler: async (
    ...
    try {
      ...
      const createParsedMessage = async (messageId: string) => {
        ...
        if (config.downloadAttachments && attachments.length > 0) {
          for (const attachment of attachments) {
            try {
              ...
              logs.push(
                `Failed to download attachment ${attachment.filename}: ${error}`,
              );
            }
          }
        }
      ...
      if (config.triggerType === "webhook") {
        ...
        logs.push(
          `Gmail webhook configured with history ID: ${watchResponse.data.historyId}`,
        );
        ...
        const monitor = new GmailTriggerMonitor(
          gmail,
          userId,
          advancedQuery,
          config.maxResults,
...
class GmailTriggerMonitor {
  ...
  async startMonitoring(callback: (message: any) => Promise<void>) {
    if (this.intervalId) {
      ...
      try {
        ...
        for (const item of messages) {
          ...
          try {
            ...
            this.logs.push(
              `Error processing message ${item.id}: ${messageError}`,
            );
        ...
        if (this.retryOnError && this.retryCount < this.maxRetries) {
          this.retryCount++;
          this.logs.push(
            `Retrying in ${this.checkInterval * 2}ms (attempt ${this.retryCount}/${this.maxRetries})`,
          );
