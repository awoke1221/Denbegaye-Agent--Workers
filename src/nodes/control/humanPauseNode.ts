...
export async function humanPauseHandler(context: any) {
  const executionId = context.executionId;
  const nodeId = context.nodeId;
  const question =
    context.config?.question || "Please review and approve to continue";
  ...
  if (
    ...
    return {
      success: false,
      error:
        "human-pause requires an executionId and cannot run outside of a tracked execution context",
      output: {
  ...
  try {
    ...
    if (fetchError) {
      logger.error("Failed to fetch execution for pause", {
        error: fetchError,
      });
      return {
        success: false,
    ...
    if (updateError) {
      logger.error("Failed to update execution status to awaiting_approval", {
        error: updateError,
      });
      return {
        success: false,
    ...
    try {
      emitSocketEvent("execution-paused", {
        executionId,
        nodeId,
        question,
        options,
      ...
      logger.error("Failed to emit execution-paused socket event", {
        error: socketError,
        executionId,
      });
      ...
      const pollForResponse = async () => {
        ...
        if (elapsedTime >= timeoutMs) {
          ...
          try {
            ...
            // Emit timeout event
            try {
              emitSocketEvent("execution-timeout", {
                executionId,
                nodeId,
              });
            } catch (socketError) {
              logger.error("Failed to emit execution-timeout socket event", {
                error: socketError,
                executionId,
              });
            }
          } catch (error) {
            logger.error("Failed to update execution status to timed_out", {
              error,
            });
          }
        ...
        try {
          ...
          if (error) {
            ...
            logger.warn("Error polling execution for human response", {
              error,
              executionId,
            });
          ...
          if (metadata.humanResponse != null) {
            ...
            try {
              ...
              logger.error("Failed to update execution after approval", {
                error: updateErr,
                executionId,
              });
            }
          ...
          logger.warn("Error during execution polling", { error: queryError });
    ...
    logger.error("Unexpected error in human-pause handler", { error });
    return {
      success: false,
